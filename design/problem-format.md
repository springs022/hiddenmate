...
- `plies`（必須）: 探索する最大手数。協力系では指定手数以下の解を短い順に列挙する。最善詰ではこの手数以内の解を検討する。
- `rule`（省略可）: 検討ルール。省略時は`helpmate`。
  - `helpmate`: 協力詰。攻方が受方玉を詰める。
  - `helpSelfmate`: 協力自玉詰。受方が攻方玉を詰める。
  - `bestMate`: 最善詰。攻方は詰みまでの手数を最小化し、受方は不詰を優先し、詰む場合は手数を最大化する。
...
最善詰の結果には、攻方最短・受方最長での手数と、その最長手数になる受方応手の変化を返す。最長手数にならない受方応手は表示しない。最大解数に達すると変化表示は省略されるが、受方の全応手を検証してから詰みと判定する。

Web版では最善詰を選ぶと「同手数駒余りの変化を表示しない」を指定できる。有効にすると、最長手数になる受方応手のうち、その応手以下のいずれかの最善手順で攻方に持駒が残る分岐を表示から除く。表示対象として残った受方分岐では、攻方着手の分岐を省略しない。この設定は問題JSONには含めない表示・検討オプションである。
