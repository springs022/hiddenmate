...
覆面駒と、駒種を指定した透明駒を使用する協力詰・協力自玉詰・最善詰を検討するソフトウェアです。駒種も不明な通常の透明駒機能は現在開発中です。
...
「透明駒（駒種指定）」では、攻方・受方ごとに透明駒の駒種と枚数を指定します。位置は盤上または所属側の駒台から推論され、透明駒の着手は`X`、可視駒を取る着手は`76X`のように表示されます。現在は合計2枚までです。フォームの初期局面は双玉を盤上に置き、残りの全駒を受方持駒にした駒箱なしの状態です。

## 現在の実装状況

現在は、覆面駒と「透明駒（駒種指定）」を使用した協力詰・協力自玉詰・最善詰の検討に対応しています。最善詰では攻方最短・受方不詰優先／最長として全応手を検証します。位置と駒種の両方が不明な通常の透明駒は開発中です。
...
cargo run -p hiddenmate_cli -- ../examples/known-invisible-best-mate.json
```
